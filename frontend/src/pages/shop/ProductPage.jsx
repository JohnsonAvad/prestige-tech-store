...
export default function ProductPage() {
  ...
  const [wishlisted, setWishlisted] = useState(() => {
    ...
    if (!saved || !product) return false;
    return JSON.parse(saved).some((p) => p.id === product?.id);
  });
    ...
    api
      ...
      .then((res) => {
        ...
        if (res.data.product?.categoryId) {
          api
            .get(`/products?category=${res.data.product.categoryId}&limit=4`)
            .then((r) =>
              setRelated(r.data.products?.filter((p) => p.id !== id) || []),
            );
        }
      })
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {product.isNewArrival && (
                <span className="bg-green-500/10 border border-green-500/30 text-green-400 text-xs font-bold px-2 py-0.5 rounded-lg">
                  New Arrival
                </span>
              )}
              {product.isFeatured && (
                <span className="bg-blue-500/10 border border-blue-500/30 text-blue-400 text-xs font-bold px-2 py-0.5 rounded-lg">
                  Featured
                </span>
              )}
              {discount > 0 && (
                <span className="bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-bold px-2 py-0.5 rounded-lg">
                  {discount}% OFF
                </span>
              )}
            </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex">
                  {Array.from({ length: 5 }, (_, i) => (
                    <svg
                      key={i}
                      className={`w-4 h-4 ${i < Math.floor(product.averageRating) ? "text-yellow-400" : "text-white/20"}`}
                      viewBox="0 0 24 24"
                      fill="currentColor"
                    >
              ...
              {product.specs && Object.keys(product.specs).length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {Object.entries(product.specs).map(([key, value]) => (
                    ...
                    >
                      <span className="text-white/40 text-sm w-32 shrink-0">
                        {key}
                      </span>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="flex">
                          {Array.from({ length: 5 }, (_, i) => (
                            <svg
                              key={i}
                              className={`w-3 h-3 ${i < review.rating ? "text-yellow-400" : "text-white/20"}`}
                              viewBox="0 0 24 24"
                              fill="currentColor"
                            >
                              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
