...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    setLoading(true)
    try {
      const res = await fetch('https://prestige-tech-store-api.vercel.app/api/auth/register', {
        method: 'POST',
    ...
    <div style={{ minHeight: '100vh', background: '#f0f9ff' }}>
      ...
      <div style={{ maxWidth: '440px', margin: '48px auto', padding: '24px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ fontSize: '14px', color: '#64748b' }}>Join Prestige TechStore today</p>
        </div>
          ...
          <div style={{ background: 'white', padding: '24px' }}>
            ...
            <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <input name="name" value={form.name} onChange={handleChange} placeholder="Enter your full name" required style={inputStyle} />
              </div>

              <div>
                <label style={labelStyle}>Email Address *</label>
                <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="Enter your email" required style={inputStyle} />
              </div>

              <div>
                <label style={labelStyle}>Phone Number *</label>
                <input name="phone" value={form.phone} onChange={handleChange} placeholder="e.g. 0701234567" required style={inputStyle} />
              </div>
              ...
              <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '12px', padding: '12px 16px' }}>
                <p style={{ fontSize: '12px', fontWeight: 700, color: '#1d4ed8', marginBottom: '8px' }}>By creating an account you get:</p>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                    ...
                    <span style={{ fontSize: '12px', color: '#0f172a' }}>{benefit}</span>
                  </div>
                ))}
              </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', justifyContent: 'center', background: '#eff6ff', borderRadius: '10px', padding: '10px 14px', border: '1px solid #bfdbfe', marginTop: '16px' }}>
          ...
          <span style={{ fontSize: '11px', color: '#1d4ed8', fontWeight: 600 }}>Your information is secure and never shared</span>
        </div>
